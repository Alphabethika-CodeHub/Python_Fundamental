# Review String Method

highlighted_poems = "Afterimages:Audre Lorde:1997,  The Shadow:William Carlos Williams:1915, Ecstasy:Gabriela " \
                    "Mistral:1925,   Georgia Dusk:Jean Toomer:1923,   Parting Before Daybreak:An Qi:2014, The Untold " \
                    "Want:Walt Whitman:1871, Mr. Grumpledump's Song:Shel Silverstein:2004, Angel Sound Mexico " \
                    "City:Carmen Boullosa:2013, In Love:Kamala Suraiyya:1965, Dream Variations:Langston Hughes:1994, " \
                    "Dreamwood:Adrienne Rich:1987 "

highlighted_poems_list = highlighted_poems.split(",")
highlighted_poems_stripped = []
highlighted_poems_details = []

for letter in highlighted_poems_list:
    highlighted_poems_stripped.append(letter.strip())

for letter in highlighted_poems_stripped:
    highlighted_poems_details.append(letter.split(":"))

titles = []
poets = []
dates = []
length_of_poem = len(highlighted_poems_details)
index = 0

for letter in highlighted_poems_details:
    titles.append(letter[0])
    poets.append(letter[1])
    dates.append(letter[2])

while index <= length_of_poem - 1:
    print("The poem {title} was published by {poet} in {date}.".format(title=titles[index], poet=poets[index],
                                                                       date=dates[index]))
    index += 1
